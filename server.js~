...
//invoke when socket is disconnected
  ...
  socket.on('setPurchaseInst',function(custID,totalPrice,numItems){setPurchaseInst(socket,custID,totalPrice,numItems);});
...
var purchaseInstID;
function setPurchaseInst(socket,custID,totalPrice,numItems){
 var time = new Date();
 var dt = time.toYMD();
 console.log(dt);
  //console.log("setPurchaseInst: " +cart[i][j].itemID);
  connection.query('INSERT INTO Purchase_Inst (purchaseDate,totalAmnt,numItems,custID) VALUES (\''+dt+'\','+totalPrice+','+numItems+','+custID+') ON DUPLICATE KEY UPDATE custID='+custID, function(err, result) {
    if (err) throw err;
